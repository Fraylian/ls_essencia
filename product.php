    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20">
        ...
        <div class="space-y-6">
            <div class="relative bg-white rounded-2xl overflow-hidden shadow-xl aspect-square group">
                <img id="mainImage" src="<?php echo $product['images'][0]; ?>" alt="<?php echo $product['name']; ?>" class="w-full h-full object-cover transition duration-500">
                <div class="absolute inset-0 ring-1 ring-inset ring-black/5 rounded-2xl"></div>
            </div>
            
            <div class="grid grid-cols-3 gap-4">
                <?php foreach ($product['images'] as $index => $img): ?>
                    <div class="relative rounded-xl overflow-hidden cursor-pointer bg-white aspect-square hover:ring-2 ring-brand transition" onclick="changeImage(this.querySelector('img'))">
                         <img src="<?php echo $img; ?>" class="w-full h-full object-cover hover:scale-110 transition duration-300 opacity-80 hover:opacity-100">
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <div class="flex flex-col justify-center">
            <h1 class="font-serif text-4xl md:text-5xl font-bold text-brand-dark mb-4 leading-tight"><?php echo $product['name']; ?></h1>
            
            <div class="flex items-center space-x-4 mb-8">
                <span class="text-3xl font-light text-brand">RD$ <?php echo number_format($product['price'], 2); ?></span>
                <span class="bg-pink-sec/20 text-brand px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">En Stock</span>
            </div>
            ...
            <form action="cart_actions.php" method="POST" class="mt-auto pt-6 border-t border-gray-200">
                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    <div class="flex items-center border border-gray-300 rounded-full px-4 py-2 bg-white w-max">
                        ...
                        <input type="number" name="quantity" id="quantity" value="1" min="1" max="10" class="w-12 text-center text-lg font-bold text-brand focus:outline-none bg-transparent">
                    </div>
                    
                    <button type="submit" class="flex-1 bg-brand text-white text-lg font-bold py-3 px-8 rounded-full shadow-lg hover:bg-brand-dark hover:shadow-brand/30 transition transform hover:-translate-y-1 flex justify-center items-center gap-2">
